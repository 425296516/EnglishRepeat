...
import com.baidu.android.voicedemo.kedaxunfei.FinalResult;
import com.baidu.android.voicedemo.kedaxunfei.Result;
import com.baidu.android.voicedemo.kedaxunfei.xml.XmlResultParser;
...
public class IseDemo extends Activity implements OnClickListener {
	...
	private EvaluatorListener mEvaluatorListener = new EvaluatorListener() {
		...
		public void onResult(EvaluatorResult result, boolean isLast) {
			Log.d(TAG, "evaluator result :" + result.getResultString() + " isLast=" + isLast);
			...
			if (isLast) {
				...
				if(!TextUtils.isEmpty(builder)) {
					...
					if (!TextUtils.isEmpty(mLastResult)) {
						...
						Result content = resultParser.parse(mLastResult);

						if (null != content) {
							Log.d(TAG,"content="+((FinalResult)content).total_score+"");
							//Log.d(TAG,content.category+""+content.total_score+content.content+content.except_info+content.language+content.beg_pos+content.end_pos+content.sentences+content.time_len);
							mResultEditText.setText("总分 : "+((FinalResult)content).total_score*20);
						} else {
							showTip("结析结果为空");
						}
					}
		...
		public void onError(SpeechError error) {
			...
			if(error != null) {	
				...
				mResultEditText.setHint("请点击“开始评测”按钮,根据发音的准确度进行评分");
			} else {
