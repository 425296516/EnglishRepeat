...
public class ApiActivity extends Activity implements RecognitionListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        test.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                startActivity(new Intent(ApiActivity.this,IseDemo.class));
    ...
    //private String content =  "i have a dream.so even though we face the difficulties of today and tomorrow,i still have a dream.it is a dream deeply rooted in the American dream.";

    @Override
    public void onResults(Bundle results) {
        SpannableStringBuilder builder = new SpannableStringBuilder(mInput.getText().toString());
        ...
        String str = nbest.get(0) + strEnd2Finish;
        ...
        try {
            ...
            strList.retainAll(contentList);

            for (int i = 0; i < strList.size(); i++) {
                Log.d(TAG, "-----" + strList.get(i));

                count++;

                num = mInput.getText().toString().toLowerCase().indexOf(strList.get(i).toLowerCase(), num);
                Log.d(TAG, strList.get(i) + "===" + num);

                ForegroundColorSpan redSpan = new ForegroundColorSpan(Color.RED);
                try{

                    builder.setSpan(redSpan, num, num+strList.get(i).length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

                }catch (Exception e){
                    Log.d(TAG,""+e.toString());
                }

                num = num + strList.get(i).length();

            }

        /*    for (int i = 0; i < s.length; i++) {
                if (Arrays.asList(content.split(" |,|\\?|\\.")).contains(s[i].toLowerCase())) {
                    content = content.replaceFirst(s[i], "");
                    Log.d(TAG, content + "");
                    //txtResult.append(s[i] + " ");

                    count++;

                    num = mInput.getText().toString().toLowerCase().indexOf(s[i].toLowerCase(), num);
                    ...
                    try{
                        ...
                        builder.setSpan(redSpan, num, num + s[i].length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            ...
            if (count < WordCounts.countNum(mInput.getText().toString())) {
                txtResult.append("\n" + "识别率为: " + ((int) (count * 10000.0 / WordCounts.countNum(mInput.getText().toString())) / 100) + "%");

            } else {
                txtResult.append("\n" + "识别率为: " + ((int) (WordCounts.countNum(mInput.getText().toString()) * 10000.0) / count / 100) + "%");

            }

            //Toast.makeText(this, nbest.get(0) + strEnd2Finish + "", Toast.LENGTH_SHORT).show();
